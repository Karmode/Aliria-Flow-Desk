...
def show():
    ...
    lista_inicial = cotizacion.get("materiales_lista", []) or []
    df_inicial_data = []
    ...
    # NOTA: NO usamos st.form aquí a propósito. Dentro de un formulario,
    # Streamlit no vuelve a ejecutar el script hasta pulsar "Guardar", por lo
    # que los totales y la tabla de materiales no se actualizarían al editar.
    ...
    with col1:
        ...
        if editable:
            nombre_cliente_seleccionado = st.selectbox(
                ...
                index=list(clientes_dict.keys()).index(nombre_cliente_actual) if nombre_cliente_actual in clientes_dict else 0
            )
        else:
    ...
    descripcion = st_quill(
        ...
        key=f"descripcion_editar_{cotizacion_id}",
    )
    ...
    # data_editor con `key` namespaced por cotización: Streamlit conserva las
    # ediciones entre reruns y, al estar FUERA de un form, cada cambio dispara
    # un rerun que recalcula los totales en vivo.
    edited_df = st.data_editor(
        df_inicial,
        num_rows="dynamic",
        use_container_width=True,
        disabled=not editable,
        key=f"materiales_editor_{cotizacion_id}",
        column_config={
    ...
    # ───────────────── RESUMEN EN VIVO ─────────────────
    materiales_total_preview = materiales_total_editor if materiales_total_editor > 0 else float(materiales_manual)
    subtotal_preview = float(mano_obra) + materiales_total_preview
    descuento_preview = float(descuento) if descuento > 0 else 0.0
    total_preview = subtotal_preview - descuento_preview

    st.markdown("#### Resumen")
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Materiales", f"${materiales_total_preview:,.0f}")
    m2.metric("Subtotal", f"${subtotal_preview:,.0f}")
    m3.metric("Descuento", f"-${descuento_preview:,.0f}")
    m4.metric("Total General", f"${total_preview:,.0f}")
    ...
    # ───────────────── BOTÓN ─────────────────
    submitted = st.button("💾 Guardar Cambios", use_container_width=True, type="primary")
    ...
    if submitted:
        ...
        df_a_guardar = edited_df.copy() if edited_df is not None else pd.DataFrame()
        materiales_lista = []
